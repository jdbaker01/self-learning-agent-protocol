...
- **M6 — Eval harness.** ✅ **Done.**
  Ships `npm run sim` — a multi-session simulator driving real `runChatTurn` with an LLM user-simulator plus a rubric-based judge. Two personas (`allergic_alex`, `deadline_dan`) are defined in `src/sepl/personas.ts` with latent facts, topic seeds, and per-dimension rubrics (safety / personalization / efficiency). For each persona the harness creates a fresh agent, runs N sessions of K user turns each, scores every session 0–10 per dimension, drives the full SEPL Learn loop between sessions, and reports first→last score deltas plus a commit timeline.

  First full run (2 personas × 3 sessions × 3 turns, `reports/sim-2026-04-21.txt`): 0/2 personas improved. Breakdown:
  - `allergic_alex` scored 9.67→8.67→9.67 overall — a **ceiling effect**. The bootstrap prompt + memory tool already handles vegetarian + peanut-allergy constraints immediately, so SEPL has little room. Rare regressions (session 2) also recover naturally.
  - `deadline_dan` scored 5.00→5.00→5.00 flat — a **retrieval miss**. The agent persisted durable facts across sessions but the M3 top-k-on-current-user-message retrieval doesn't surface them when the user's later message is on a different topic than the original fact. Three SEPL commits across the run didn't move the score because the underlying gap is in retrieval breadth, not the prompt or memory contents.

  Both findings are real signal, not harness bugs: the starter bootstrap is stronger than the personas stress, and M3's retrieval is too narrow for cross-session personalization when the current turn's wording drifts. Plausible M6+ follow-ups: (a) a "weak bootstrap" mode so the harness has headroom to measure; (b) widen retrieval to include session-level context, not just the last user message; (c) more adversarial personas that actively hide facts and spring traps in later sessions.
